...
public enum DynamicVariant {
    ...
    public static final DynamicVariant[] VALUES = values();
    ...
    DynamicVariant(Class<? extends BlockCamo> block, Class<? extends TileEntity> tile, String name) {
        this.block = block;
        this.tile = tile;
        this.name = name;
    }

    public static boolean matches(DynamicVariant type, ItemStack stack) {
        return fromStack(stack).equals(type);
    }

    public static DynamicVariant fromName(String name) {
        return Arrays.stream(VALUES).filter(t -> t.isName(name)).findFirst().orElse(null);
    }

    public static DynamicVariant fromBlock(BlockCamo block) {
        return Arrays.stream(VALUES).filter(t -> t.isBlock(block)).findFirst().orElse(null);
    }

    public static DynamicVariant fromStack(ItemStack stack) {
    ...
    public static void registerTiles() {
        for (DynamicVariant type : VALUES) {
            if (type.tile != null) {
    ...
    public Class<? extends BlockCamo> getBlock() {
        return block;
    }
