...
public class DummySoulUrnRecipe extends SoulUrnRecipe {
    ...
    public DummySoulUrnRecipe(SoulUrnRecipe parent) {
        super(StackIngredient.fromIngredient(8, parent.input), parent.getOutputs(), parent.getSecondaryOutputs().stream().map(s -> InventoryUtils.setCount(s, 8)).collect(Collectors.toList()));
    }
