    ...
    public void onInit(FMLInitializationEvent event) {
        if (BetterWithMods.MODULE_LOADER.isFeatureEnabled(MetalReclaming.class)) {
            ...
            RecipeRegistry.CRUCIBLE.registerAll(
                    builder.stoked().inputs(new ItemStack(BWMBlocks.DETECTOR_RAIL_STONE, 6)).outputs(new ItemStack(Items.IRON_INGOT, 6)).build(),
                    builder.stoked().inputs(new ItemStack(BWMBlocks.DETECTOR_RAIL_STEEL, 6)).outputs(new ItemStack(Items.IRON_INGOT, 6), ItemMaterial.getStack(ItemMaterial.EnumMaterial.STEEL_INGOT, 2)).build()
