...
public class BWMOreDictionary {
    ...
    private static boolean isPlank(ItemStack output) {
        return BWMOreDictionary.listContains(output, OreDictionary.getOres("plankWood"));
    }
    ...
    public static void findLogRecipes() {
        BWMOreDictionary.logs = OreDictionary.getOres("logWood").stream().filter(s -> !s.isEmpty()).collect(Collectors.toList());
